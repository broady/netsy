...
func TestApplyTxnReproducesSwallowedLeaderTxnError(t *testing.T) {
	...
	resp, err := clientSrv.ApplyTxn(context.Background(), &pb.TxnRequest{
		Compare: []*pb.Compare{{
			Key:    []byte("key"),
	...
	if err != nil {
		t.Fatalf("ApplyTxn() error = %v, want nil to reproduce swallowed LeaderTxn error", err)
	}
	if resp == nil {
		t.Fatal("ApplyTxn() response = nil, want fallback response")
	}
	if resp.Succeeded {
		t.Fatal("ApplyTxn() returned Succeeded=true; want false fallback response")
	}
}
